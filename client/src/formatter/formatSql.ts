 ...
 * formatSql
 *
 * The whole formatting pipeline: sql-formatter for the base layout, then the
 * passes that bring the result in line with the SQL Prompt style options.
 *
 * It lives outside the VS Code provider on purpose — no `vscode` import — so
 * the pipeline that ships is exactly the one the example fixtures exercise.
 */
...
import { collapseCaseToSingleLine, applyCaseFormatting } from './caseFormatter';
...
export function formatSql(text: string, options: SqlPromptStyleJson): string {
