CREATE PROCEDURE rf.spLoadEntity
    (
        @codeA VARCHAR(3), @codeB VARCHAR(3), @idRow INT, @qty NUMERIC(12, 3), @unitCode VARCHAR(50) = 'UNITX', @writeAction BIT, @dtStart DATETIME, @dtEnd DATETIME, @reasonCode VARCHAR(5)
      , @userCode VARCHAR(10), @terminal AS NUMERIC(3, 0), @weight AS NUMERIC(12, 3) = 0, @docInfo AS VARCHAR(255) = '', @serial AS NVARCHAR(100) = '', @mode VARCHAR(5) = 'STD'
      , @slot VARCHAR(25) = NULL, @slotUnit VARCHAR(50) = NULL, @blockCode VARCHAR(10) = NULL, @hostCode VARCHAR(50) = NULL, @Error VARCHAR(255) OUTPUT
    )
AS
    SET ANSI_NULLS ON;
