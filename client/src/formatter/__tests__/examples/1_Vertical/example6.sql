...
AS
    BEGIN
        ...
        DECLARE @codeA    VARCHAR(3)     = 'AAA'
              , @codeB    VARCHAR(3)     = '001'
              , @userCode VARCHAR(10)    = 'user01'
              , @terminal INT            = 99
              , @typeCode VARCHAR(1)     = 'X'
              , @nameCode VARCHAR(10)    = 'name01'
              , @docType  VARCHAR(3)     = 'DOC'
              , @docYear  SMALLINT       = YEAR(GETDATE())
              , @docNum   VARCHAR(20)    = '10001'
              , @docRow   VARCHAR(30)    = '1'
              , @itemCode VARCHAR(40)    = 'ITEM_SAMPLE'
              , @Error    VARCHAR(255)
              , @dtValue  DATETIME       = GETDATE()
              , @idValue  INT            = 78
              , @qtyValue DECIMAL(12, 3) = 2.152
              , @reasonA  VARCHAR(5)     = IIF(@isMode = 1, 'R_A', 'R_B')
              , @reasonB  VARCHAR(5)     = 'R_C'
              , @reasonC  VARCHAR(5)     = 'R_D'
              , @lotCode  NVARCHAR(20)   = N'LOT001'
              , @dtLot    DATE           = '20170612'
              , @dtExp    DATE           = '20180312';
        ...
        INSERT dbo.TABLE_TYPE ( COL_TYPE
                              , COL_MODE )
        VALUES ( @docType, 2 );

        INSERT INTO dbo.TABLE_ITEM ( COL_ITEM
                                   , COL_DESC )
        VALUES ( @itemCode, 'DESC_SAMPLE' );

        INSERT INTO dbo.TABLE_HEAD ( COL_CODE_A
                                   , col_code_b
                                   , COL_TYPE
                                   , COL_NUM
                                   , COL_STATE
        ...
        INSERT INTO dbo.TABLE_DETAIL ( COL_CODE_A
                                     , col_code_b
                                     , COL_TYPE
                                     , COL_NUM
                                     , COL_ROW
        ...
        INSERT INTO dbo.TABLE_NAME ( COL_TYPE
                                   , COL_NUM
                                   , COL_NAME )
